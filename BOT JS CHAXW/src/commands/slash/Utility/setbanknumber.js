...
module.exports = {
    structure: new SlashCommandBuilder()
        .setName('setbanknumber')
        .setDescription('Set a new bank number for a character')
        ...
        .addStringOption(option =>
            option.setName('newnumber')
                .setDescription('New bank number')
                .setRequired(true)),
    /**
    ...
    run: async (client, interaction) => {
      ...
      return interaction.reply({
        ...
        db.query('SELECT * FROM players WHERE username = ?', [username], (err, results) => {
            ...
            db.query('UPDATE players SET brek = ? WHERE username = ?', [newNumber, username], (err) => {
                if (err) {
                    console.error(err);
                ...
                interaction.reply({ content: `Successfully set new bank number for ${username}.` });
            });
        });
    }
};
