...
module.exports = {
    structure: new SlashCommandBuilder()
        .setName('setmoney')
        .setDescription('Set money for a character')
    ...
    run: async (client, interaction) => {
      ...
      return interaction.reply({
        ...
        db.query('SELECT * FROM players WHERE username = ?', [username], (err, results) => {
            ...
            db.query('UPDATE players SET money = ? WHERE username = ?', [amount, username], (err) => {
                if (err) {
                    console.error(err);
                ...
                interaction.reply({ content: `Successfully set ${amount} money for ${username}.` });
            });
        });
    }
};
