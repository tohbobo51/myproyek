CMD:tag(playerid, params[])
{
	...
	if (sscanf(params, "s[7]", options))
		return Syntax(playerid, "/tag [create/track/clean/info]");
	...
	if(!strcmp(options, "create", true))
	{
        ...
        if(!PlayerHasItem(playerid, "Pilox"))
            return Error(playerid, "Anda tidak memiliki Pilox untuk membuat graffiti!");
		...
		if(IsPlayerEditingTags(playerid))
			return Syntax(playerid, "Selesaikan proses pembuatan tag terlebih dahulu!");
	...
	else if (!strcmp(options, "clean", true))
	{
		...
		{
			if((AccountData[playerid][pFaction] != FACTION_POLISI) && (AccountData[playerid][pFaction] != FACTION_GOJEK) &&(AccountData[playerid][pFaction] != FACTION_PEMERINTAH) && (TagsData[index][tagPlayerID] != AccountData[playerid][pID]) && (GetAdminLevel(playerid) < 5))
				return ShowPlayerFooter(playerid, "~r~ERROR: ~w~Ini bukan tag milikmu!.");

			Tags_Delete(index);
			Info(playerid, "Spray tag didekatmu telah dihapus!");
		}
	...
	else if (!strcmp(options, "track", true))
	{
		...
		foreach(new i : Tags) if(TagsData[i][tagPlayerID] == AccountData[playerid][pID])
	...
	else Syntax(playerid, "/tag [create/track/clean/info]");
	return 1;
}