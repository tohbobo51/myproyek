...
Tags_Create(playerid)
{
	...
	if((index = Iter_Free(Tags)) != cellmin)
	{
		...
		format(TagsData[index][tagPlayerName], MAX_PLAYER_NAME, ReturnNames(playerid, 0));
		...
		TagsData[index][tagPlayerID] = AccountData[playerid][pID];
		...
		new output[700];
		mysql_format(g_SQL, output, sizeof(output), "INSERT INTO `tags`(`tagText`, `tagFont`, `tagCreated`, `tagColor`, `tagFontsize`, `tagBold`, `tagOwner`, `tagPosx`, `tagPosy`, `tagPosz`, `tagRotx`, `tagRoty`, `tagRotz`, `tagExpired`) VALUES ('%e','%e','%e','%d','%d','%d','%d','%.3f','%.3f','%.3f','%.3f','%.3f','%.3f','%d')", 
...
Tags_Delete(index)
{
	...
	{
		mysql_tquery(g_SQL, sprintf("DELETE FROM `tags` WHERE `tagId`='%d';", TagsData[index][tagID]));
...
Tags_Sync(index)
{
	if(Tags_IsExists(index))
	{
		...
		else TagsData[index][tagObjectID] = CreateDynamicObject(18661, TagsData[index][tagPosition][0], TagsData[index][tagPosition][1], TagsData[index][tagPosition][2], TagsData[index][tagRotation][0], TagsData[index][tagRotation][1], TagsData[index][tagRotation][2], 0, 0, -1, 100.0, 100.0);
...
Tags_GetCount(playerid)
{
	...
	foreach(new i : Tags)
	{
		if(TagsData[i][tagPlayerID] != AccountData[playerid][pID])
			continue;
	...
	if(response)
	{
		if(IsPlayerEditingTags(playerid))
		{
			switch(listitem)
			{
				...
				{
					Info(playerid, "Posisikan tulisan spray, pastikan tidak jauh "ORANGE"5 meter "WHITE"darimu!");
					EditDynamicObject(playerid, editing_object[playerid]);
				}
				case 1: // Editing Text
				{
                    Dialog_Show(playerid, TagsText, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Text", 
                    ...
                    \n(b) = membuat warna text hitam\
                    \n(y) = membuat warna text kuning\
                    \n(bl) = membuat warna text biru\
                    \n(g) = membuat warna text hijau\
                    \n(o) = membuat warna text orange\
                    \n(w) = membuat warna text putih\
                    \n\nNOTE: Jika ingin menggunakan warna yang ada pada Font Color tidak usah menginput warna dari pemberitahuan diatas", "Change", "Back");
                }
				case 2: // Font Name
				{
					Dialog_Show(playerid, TagsFont, DIALOG_STYLE_LIST, ""TTR"Aeterna Roleplay "WHITE"- Font Name", object_font, "Change", "Back");
				}
				case 3: // Font Name
				{
					Dialog_Show(playerid, TagsFontSize, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Font Size", WHITE"Ukuran sekarang: "YELLOW"%d\n\n"WHITE"Masukkan ukuran font mulai dari angka 1 sampai "#TAGS_DEFAULT_MAX_SIZE":", "Update", "Back", GetPVarInt(playerid, "TagsSize"));
				}
				case 4: // Font Color
				{
					Dialog_Show(playerid, TagsColor, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Font Color", color_string, "Change", "Back");
				}
				case 5: // Toggle bold
				{
					SetPVarInt(playerid, "TagsBold", !GetPVarInt(playerid, "TagsBold"));
					Info(playerid, "Tulisan berganti menjadi "YELLOW"%s", GetPVarInt(playerid, "TagsBold") ? ("bold") : ("reguler"));
...
Dialog:TagsText(playerid, response, listitem, inputtext[])
{
	...
	{
		if(isnull(inputtext))
			return Dialog_Show(playerid, TagsText, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Text", WHITE"error: text tidak boleh kosong!!\n\nMasukkan text untuk ditampilkan pada spray tag:\n\nFormat code:\n- (n): untuk membuat baris baru | (b): memberi warna biru | (bl): memberi warna hitam | (g): memberi warna hijau\n- (r): memberi warna merah | (y): memberi warna kuning | (w): memberi warna putih", "Change", "Back");

		if(strlen(inputtext) > TAGS_TEXT_LENGTH)
			return Dialog_Show(playerid, TagsText, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Text", WHITE"error: text hanya dibatasi 1 - "#TAGS_TEXT_LENGTH" karakter!\n\nMasukkan text untuk ditampilkan pada spray tag:\n\nFormat code:\n- (n): untuk membuat baris baru | (b): memberi warna biru | (bl): memberi warna hitam | (g): memberi warna hijau\n- (r): memberi warna merah | (y): memberi warna kuning | (w): memberi warna putih", "Change", "Back");
...
Dialog:TagsFont(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(listitem == sizeof(FontNames) - 1)
			return Dialog_Show(playerid, TagsFontCustom, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Custom Font", "Masukkan nama font yang akan kamu ubah:", "Input", "Back");
...
Dialog:TagsFontCustom(playerid, response, listitem, inputtext[])
{
	...
	{
		if(!strlen(inputtext))
			return Dialog_Show(playerid, TagsFontCustom, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Custom Font", "error: tidak boleh kosong, silahkan masukkan nama font yang benar!\n\nMasukkan nama font yang akan kamu ubah:", "Input", "Back");
...
Dialog:TagsFontSize(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(!(0 < strval(inputtext) <= TAGS_DEFAULT_MAX_SIZE))
			return Dialog_Show(playerid, TagsFontSize, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Font Size", WHITE"error: ukuran dibatasi mulai dari 1 sampai "#TAGS_DEFAULT_MAX_SIZE"!\n\nUkuran sekarang: "YELLOW"%d\n\n"WHITE"Masukkan ukuran font mulai dari angka 1 sampai "#TAGS_DEFAULT_MAX_SIZE":", "Update", "Back", GetPVarInt(playerid, "TagsSize"));
...
Dialog:TagsColor(playerid, response, listitem, inputtext[])
{
	if(response)
	{
        if(!(0 <= strval(inputtext) <= sizeof(ColorList)-1))
			return Dialog_Show(playerid, TagsColor, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Font Color", color_string, "Change", "Back");
...
Dialog:TrackTags(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(Tags_IsExists(index) && TagsData[index][tagPlayerID] == AccountData[playerid][pID])
		{
			Info(playerid, "Pergi ke tujuan waypoint yang telah dibuat.");
        	SetPlayerRaceCheckpoint(playerid, 1, TagsData[index][tagPosition][0], TagsData[index][tagPosition][1], TagsData[index][tagPosition][2], 0.0, 0.0, 0.0, 5.0);
