Dialog:EditingVehObject(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else
        {
            ...
            {
                Dialog_Show(playerid, VACCSE, DIALOG_STYLE_LIST, ""TTR"Aeterna Roleplay "WHITE"- Edit Component", "Position\nPosition (Manual)\nChange Color\nRemove Modification\nSave", "Select", "Back");
            }
            else if(VehicleObjects[vehicleid][slot][vehObjectType] == OBJECT_TYPE_TEXT)
            {
                Dialog_Show(playerid, VACCSE1, DIALOG_STYLE_LIST, ""TTR"Aeterna Roleplay "WHITE"- Edit Component", "Position\nPosition (Manual)\nText Name\nText Size\nText Font\nText Color\nRemove Modification\nSave", "Select", "Back");
            }
            else if(VehicleObjects[vehicleid][slot][vehObjectType] == OBJECT_TYPE_LIGHT)
            {
               Dialog_Show(playerid, VACCSE2, DIALOG_STYLE_LIST, ""TTR"Aeterna Roleplay "WHITE"- Edit Component", "Position\nPosition (Manual)\nRemove Modification\nSave", "Select", "Back");
            }
        }
    }
    return 1;
...
Dialog:MODSHOP_CUSTOM(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(isnull(inputtext)) return Dialog_Show(playerid, MODSHOP_CUSTOM, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Modshop",
        ...
        if(!IsNumeric(inputtext)) return Dialog_Show(playerid, MODSHOP_CUSTOM, DIALOG_STYLE_INPUT, ""TTR"Aeterna Roleplay "WHITE"- Modshop",
        ...
        SendCustomMessage(playerid, "MODSHOP", "Anda membeli Custom Aksesoris Seharga "GREEN"$30,000");
    }   
    return 1;
}
        ...
        switch(listitem)
        {
            case 0:
			{
				if(VehicleObjects[vehicleid][slot][vehObjectType] == OBJECT_TYPE_BODY)
				{
                    ...
                    SendCustomMessage(playerid, "MODSHOP", "Anda dapat mengedit posisi object sekarang");
				}
				else
				{
                    SetVehicleZAngle(GetPlayerVehicleID(playerid), 0.0);
            ...
            case 3:
            {
                ...
                SendCustomMessage(playerid, "MODSHOP", "Anda menghapus "YELLOW"%s"WHITE" dari kendaraan anda!", GetVehObjectNameByModel(modelid));
            }
            case 4:
            {
                Vehicle_ObjectSave(vehicleid, slot);
                callcmd::buymod(playerid, "");
...
Dialog:VACCSE1(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        switch(listitem)
        {
            ...
            {
                Vehicle_ObjectDelete(vehicleid, slot);
                SendCustomMessage(playerid, "MODSHOP", "Anda telah menghapus Text Object dari kendaraan!");
            }
            case 7:
            {
                Vehicle_ObjectSave(vehicleid, slot);
...
Dialog:DIALOG_MSEDITX(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        AttachDynamicObjectToVehicle(VehicleObjects[vehicleid][slot][vehObject], PlayerVehicle[vehicleid][pVehPhysic], 
        posisi, 
