...
hook OnPlayerConnect(playerid)
{
    ...
    NoClipPos[playerid][0] = 0;
    NoClipPos[playerid][1] = 0;
    NoClipPos[playerid][2] = 0;
	return 1;
}

//--------------------------------------------------

CMD:noclip(playerid, params[])
{
    if (!AccountData[playerid][IsLoggedIn] || !AccountData[playerid][pSpawned])
        return Error(playerid, "Anda belum login!");
    ...
    if (GetPVarType(playerid, "NoClipMode"))
    {
        CancelFlyMode(playerid);
        Info(playerid, "Anda berhenti dari mode No Clip");
    }
    else 
    {
        FlyMode(playerid);
        Info(playerid, "Anda menggunakan mode No Clip");
    }
    return 1;
}
...
{
	DeletePVar(playerid, "NoClipMode");
	CancelEdit(playerid);
    ...
    NoClipPos[playerid][0] = NoClipPos[playerid][1] = NoClipPos[playerid][2] = 0;
	return 1;
}
...
stock FlyMode(playerid)
{
	...
	X = NoClipPos[playerid][0];
	Y = NoClipPos[playerid][1];
	Z = NoClipPos[playerid][2];
	...
	SetPVarInt(playerid, "NoClipMode", 1);
